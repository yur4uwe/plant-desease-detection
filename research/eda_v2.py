...
def load_before():
    conn = sqlite3.connect("data/processed/observations.db")
...
def main():
    ...
    fig.suptitle(
        "Numerical Distributions: Before vs After Cleaning (Isolation Forest + Limits)",
        fontsize=16,
    )
    ...
    sns.scatterplot(
        x="longitude", y="latitude", data=df_before, ax=axes[0], alpha=0.5, color="red"
    )
    ...
    sns.scatterplot(
        x="longitude", y="latitude", data=df_after, ax=axes[1], alpha=0.5, color="green"
    )
