...
def load_data(db_path="etl/data/processed/observations.db", syn_path="data/syn_data_gen_tsar.csv") -> pd.DataFrame:
...
def clean_data(df: pd.DataFrame, normalize: bool = True) -> pd.DataFrame:
    ...
    invalid_coords = (df_clean['latitude'] < -90) | (df_clean['latitude'] > 90) | \
    ...
    logging.info(f"Isolation Forest & Hard Rules identified {outliers.sum()} anomalies. Removing them.")
