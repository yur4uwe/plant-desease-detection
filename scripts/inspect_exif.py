...
def scan_raw_data(root_dir="etl/data/raw"):
    ...
    print(
        f"Images with EXIF:     {stats['has_exif']} ({stats['has_exif'] / stats['total_images']:.1%})"
    )
    print(
        f"Images with GPS:      {stats['has_gps']} ({stats['has_gps'] / stats['total_images']:.1%})"
    )
    print(
        f"Images with Time:     {stats['has_timestamp']} ({stats['has_timestamp'] / stats['total_images']:.1%})"
    )
    print("-" * 50)
